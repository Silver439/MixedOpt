...
def candidate_dycors(
    ...
    for i in subset:
        if i in opt_prob.cat_var:
            rankindex = np.where(opt_prob.cat_var == i)[0][0]
            rank = opt_prob.ranks[rankindex]
            # print(rank)
            ind = np.where(ar[:, i] == 1)[0]
            for j in ind:
                current_value = xbest[i]
                current_index = rank.index(current_value)
                if current_index == 0:
                    neighbors = [rank[current_index + 1]]
                elif current_index == len(rank)-1:
                    neighbors = [rank[current_index - 1]]
                else:
                    neighbors = [rank[current_index - 1], rank[current_index + 1]]
                cand[j, i] = np.random.choice(neighbors)
        else:
