...
class Controller(object):
    ...
    def best_loc(self, merit=None, filter=None):
        # 返回最优点带类别变量的的原始坐标
        if filter is None:
            fcomplete = [f for f in self.fevals if f.is_completed]
        else:
        ...
        if merit is None:
            def merit(r):
                return r.value
        if fcomplete:
            fmin = min(fcomplete, key=merit)
            loc = fmin.params[0].copy()
            currentRank = self.strategy.opt_prob.ranks
            for catindex in self.strategy.opt_prob.cat_var:
                rank_index = np.where(self.strategy.opt_prob.cat_var == catindex)[0][0]
                loc[catindex] = np.where(currentRank[rank_index] == loc[catindex])[0][0]
            return loc
...
class SerialController(Controller):
    ...
    def _run(self, merit=None, filter=None, reraise=True):
        ...
        while True:
            ...
            elif proposal.action == 'changerank' and proposal.action != 'terminate':
                proposal.accept()
